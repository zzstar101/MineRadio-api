...
use crate::librespot_core::{Error, authentication::Credentials, packet::PacketType, version};

use crate::librespot_core::protocol::keyexchange::{APLoginFailed, ErrorCode};
...
pub async fn connect(host: &str, port: u16, proxy: Option<&Url>) -> io::Result<Transport> {
    ...
    tokio::time::timeout(TIMEOUT, {
        let socket = crate::librespot_core::socket::connect(host, port, proxy).await?;
...
pub async fn authenticate(
    ...
    use crate::librespot_core::protocol::authentication::{
        APWelcome, ClientResponseEncrypted, CpuFamily, Os,
    };
    ...
    let os = match crate::librespot_core::config::OS {
