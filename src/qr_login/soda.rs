...
impl QrLogin for SodaQrLoginService {
    async fn create_key(&self) -> anyhow::Result<ProviderLoginQrKey> {
        let image = self.load_qr_image(None).await?;
    ...
    async fn create_image(&self, key: &str) -> anyhow::Result<ProviderLoginQrImage> {
        self.load_qr_image(Some(key)).await
    }
    ...
    async fn check(&self, key: &str) -> anyhow::Result<ProviderLoginQrCheck> {
        ...
        if confirmed && let Some(cookie) = cookie_from_set_cookie_headers(&headers) {
...
impl SodaQrLoginService {
    async fn load_qr_image(&self, key: Option<&str>) -> anyhow::Result<ProviderLoginQrImage> {
        ...
        if !normalized_key.is_empty() {
            ...
            return Ok(ProviderLoginQrImage {
                provider: ProviderId::Soda,
                key: normalized_key.to_owned(),
                img: session.lock().await.image.clone(),
                url: None,
            });
        }
        ...
        self.sessions
            .insert(image.key.clone(), image.img.clone(), ())
            .await;
        Ok(image)
    }
}
