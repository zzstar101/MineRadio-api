...
//!     例如: 与取址绑定的伴生事实（试听区间）必须并入响应体SongUrlResult，因为它是取址调用本身的副产物，
//!     且换源后它是实际源的数据，天然准确（溯源见 SongUrlResult.source_track）
...
//! 2. **换源决策单点发生，其它能力以 source_track 为锚**。跨源换源只允许发生在
//!    song_url（cross_source 层）——它是一次独立的搜索匹配，任何接口若自己再搜一次，
//!    两次匹配可能落在不同候选上，溯源即断裂。因此：
//!    - SongUrlResult 必须携带 source_track（实际产出 URL 的歌曲，未换源时等于请求的
//!      track），品质/歌词等后续查询由客户端以它为锚重新发起，服务端不设跨源回退；
//!    - 与取址绑定的伴生事实（如试听区间）仍并入 SongUrlResult 本体：它是取址调用
//!      本身的副产物，跟着 URL 走，换源后天然准确，拆出去只会多一次往返、还要为
//!      只在取址响应里出现的信息设计缓存透传。
